...
function createLayout(props) {
  ...
  const transitionsEnabled = computed(() => {
    return !Array.from(disabledTransitions.values()).some((ref3) => ref3.value);
  });
...
import { computed as computed2, inject as inject2, toValue, createVNode as _createVNode, createElementVNode as _createElementVNode, mergeProps as _mergeProps, normalizeClass as _normalizeClass } from "vue";
...
var useIcon = (props) => {
  const icons = inject2(IconSymbol);
...
import { inject as inject3, toRef as toRef2 } from "vue";
...
function useGoTo() {
  ...
  const goToInstance = inject3(GoToSymbol);
  const {
    isRtl
  } = useRtl();
  ...
  const goTo = {
    ...
    rtl: toRef2(() => goToInstance.rtl.value || isRtl.value)
  };
...
import { inject as inject4, reactive as reactive2, watch as watch2 } from "vue";
...
function createInstance(options, locale) {
  const instance = reactive2(typeof options.adapter === "function" ? new options.adapter({
...
function useDate() {
  const options = inject4(DateOptionsSymbol);
...
import { onScopeDispose, toValue as toValue2, watch as watch3 } from "vue";
function useHotkey(keys, callback) {
  ...
  function isInputFocused() {
    if (toValue2(inputs)) return false;
  ...
  function handler(e) {
    ...
    if (toValue2(preventDefault)) e.preventDefault();
    if (!isSequence) {
      callback(e);
      return;
    }
    ...
    timeout = window.setTimeout(resetSequence, toValue2(sequenceTimeout));
  }
  function cleanup() {
    window.removeEventListener(toValue2(event), handler);
    clearTimeout(timeout);
  }
  watch3(() => toValue2(keys), (newKeys) => {
    cleanup();
    if (newKeys) {
      ...
      keyGroups = groups;
      resetSequence();
      window.addEventListener(toValue2(event), handler);
    }
  }, {
    immediate: true
  });
  watch3(() => toValue2(event), (newEvent, oldEvent) => {
...
//# sourceMappingURL=chunk-Q3N6QX24.js.map
