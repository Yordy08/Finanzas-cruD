...
import { computed as computed4, inject as inject2, provide as provide2, ref as ref4, toRef as toRef3 } from "vue";
...
import { computed as computed3, ref as ref2, toRaw as toRaw2, watch as watch3 } from "vue";
function useProxiedModel(props, prop, defaultValue) {
  let transformIn = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : (v) => v;
  let transformOut = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : (v) => v;
  const vm = getCurrentInstance("useProxiedModel");
  const internal = ref2(props[prop] !== void 0 ? props[prop] : defaultValue);
  const kebabProp = toKebabCase(prop);
  const checkKebab = kebabProp !== prop;
  const isControlled = checkKebab ? computed3(() => {
    void props[prop];
  ...
  const model = computed3({
    ...
    set(internalValue) {
      ...
      const value = toRaw2(isControlled.value ? props[prop] : internal.value);
...
import { ref as ref3, shallowRef as shallowRef4, toRef as toRef2, watch as watch4 } from "vue";
...
function createProvideFunction(state) {
  return (props) => {
    ...
    return {
      ...
      fallback,
      messages,
      decimalSeparator: toRef2(() => inferDecimalSeparator(current, fallback)),
...
function createVuetifyAdapter(options) {
  const current = shallowRef4(options?.locale ?? "en");
  const fallback = shallowRef4(options?.fallback ?? "en");
  const messages = ref3({
    en: en_default,
    ...options?.messages
  });
  return {
    ...
    fallback,
    messages,
    decimalSeparator: toRef2(() => options?.decimalSeparator ?? inferDecimalSeparator(current, fallback)),
    t: createTranslateFunction(current, fallback, messages),
    n: createNumberFunction(current, fallback),
    provide: createProvideFunction({
      current,
...
function useLocale() {
  const locale = inject2(LocaleSymbol);
...
function createRtl(i18n, options) {
  const rtl = ref4(options?.rtl ?? genDefaults());
  const isRtl = computed4(() => rtl.value[i18n.current.value] ?? false);
  return {
    isRtl,
    rtl,
    rtlClasses: toRef3(() => `v-locale--is-${isRtl.value ? "rtl" : "ltr"}`)
  };
}
function provideRtl(locale, rtl, props) {
  const isRtl = computed4(() => props.rtl ?? rtl.value[locale.current.value] ?? false);
  return {
    isRtl,
    rtl,
    rtlClasses: toRef3(() => `v-locale--is-${isRtl.value ? "rtl" : "ltr"}`)
...
import { computed as computed5, getCurrentScope, inject as inject3, onScopeDispose as onScopeDispose2, provide as provide3, ref as ref5, shallowRef as shallowRef5, toRef as toRef4, watch as watch5, watchEffect as watchEffect3 } from "vue";
...
function createTheme(options) {
  ...
  const _name = shallowRef5(parsedOptions.defaultTheme);
  const themes = ref5(parsedOptions.themes);
  const systemName = shallowRef5("light");
  const name = computed5({
    get() {
  ...
  const computedThemes = computed5(() => {
    const acc = {};
  ...
  const current = toRef4(() => computedThemes.value[name.value]);
  const isSystem = toRef4(() => _name.value === "system");
  const styles = computed5(() => {
    const lines = [];
  ...
  const themeClasses = toRef4(() => parsedOptions.isDisabled ? void 0 : `${parsedOptions.prefix}theme--${name.value}`);
  const themeNames = toRef4(() => Object.keys(computedThemes.value));
  ...
  function install(app) {
    ...
    if (head) {
      ...
      if (head.push) {
        ...
        if (IN_BROWSER) {
          head.addHeadObjs(toRef4(getHead));
          watchEffect3(() => head.updateDOM());
        } else {
...
function provideTheme(props) {
  ...
  const theme = inject3(ThemeSymbol, null);
  if (!theme) throw new Error("Could not find Vuetify theme injection");
  const name = toRef4(() => props.theme ?? theme.name.value);
  const current = toRef4(() => theme.themes.value[name.value]);
  const themeClasses = toRef4(() => theme.isDisabled ? void 0 : `${theme.prefix}theme--${name.value}`);
  const newTheme = {
    ...theme,
    name,
    current,
...
import { computed as computed6, inject as inject4, onScopeDispose as onScopeDispose3, reactive as reactive2, shallowRef as shallowRef6, toRef as toRef5, toRefs, watchEffect as watchEffect4 } from "vue";
...
function createDisplay(options, ssr) {
  ...
  const height = shallowRef6(getClientHeight(ssr));
  const platform = shallowRef6(getPlatform(ssr));
  const state = reactive2({});
  const width = shallowRef6(getClientWidth(ssr));
  function updateSize() {
...
function useDisplay() {
  ...
  const display = inject4(DisplaySymbol);
...
//# sourceMappingURL=chunk-TFR4PTTJ.js.map
